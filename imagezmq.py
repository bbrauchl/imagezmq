...
class ImageSender():
    ...
    def __init__(self, connect_to='tcp://127.0.0.1:5555', REQ_REP=True, zmq_context=None):
        ...
        if REQ_REP == True:
             ...
             self.init_reqrep(connect_to, zmq_context)
        else:
             #PUB/SUB mode, non-blocking scenario
             self.init_pubsub(connect_to, zmq_context)

    def init_reqrep(self, address, zmq_context):
...
class ImageHub():
    ...
    def __init__(self, open_port='tcp://*:5555', REQ_REP=True, zmq_context=None):
        ...
        if REQ_REP ==True:
            ...
            self.init_reqrep(open_port, zmq_context)
        else:
            #Connect to PUB socket for non-blocking mode
            self.init_pubsub(open_port, zmq_context)
    ...
    def init_pubsub(self, address, zmq_context):
